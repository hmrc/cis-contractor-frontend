...
class WorksReferenceNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val formProvider       = new WorksReferenceNumberFormProvider()
  private val form: Form[String] = formProvider()

  private lazy val worksReferenceNumberRoute: String =
  ...
  "WorksReferenceNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = uaWithName.set(WorksReferenceNumberPage, "answer").success.value
    ...
    "must bind the form and redirect to CheckYourAnswersController page and" +
      "not add the page to AmendedPagesPage on POST when valid data is submitted in NormalMode" in {
        val onwardRoute           = controllers.add.routes.CheckYourAnswersController.onPageLoad()
        ...
        running(application) {
          ...
          updatedAnswers.get(WorksReferenceNumberPage) mustBe Some(validValue)
          updatedAnswers.get(AmendedPagesPage) mustBe None
        }
      }

    "must add WorksReferenceNumberPage to AmendedPagesPage when submitted in AmendMode" in {
      ...
      running(application) {
        val request =
          FakeRequest(
            POST,
            controllers.add.routes.WorksReferenceNumberController
              .onSubmit(AmendMode)
              .url
        ...
        val updatedAnswers = captor.getValue
        updatedAnswers.get(WorksReferenceNumberPage) mustBe Some("WR-001")
        updatedAnswers.get(AmendedPagesPage).value must contain(WorksReferenceNumberPage.toString)
      }
    }
    ...
    "must redirect to Journey Recovery for a POST when subcontractor name is missing (userAnswers present)" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, worksReferenceNumberRoute)
            .withFormUrlEncodedBody("value" -> "1234567-AB")
