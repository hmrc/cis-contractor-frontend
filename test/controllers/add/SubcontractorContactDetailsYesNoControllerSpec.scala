...
class SubcontractorContactDetailsYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  "SubcontractorContactDetailsYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, subcontractorContactDetailsYesNoRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[SubcontractorContactDetailsYesNoView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(SubcontractorContactDetailsYesNoPage, true).success.value
