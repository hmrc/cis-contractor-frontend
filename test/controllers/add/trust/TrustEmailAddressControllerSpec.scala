...
class TrustEmailAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val trustEmailAddressFinalValidationRoute: String =
    controllers.add.trust.routes.TrustEmailAddressController.onPageLoad(FinalValidationMode).url
  ...
  "TrustEmailAddressController" - {
    ...
    "must return OK for a GET in FinalValidationMode when TrustContactMethodOptions is missing" in {
      ...
      running(application) {
        val request = FakeRequest(GET, trustEmailAddressFinalValidationRoute)
    ...
    "must submit successfully in FinalValidationMode when TrustContactMethodOptions is missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, trustEmailAddressFinalValidationRoute)
            .withFormUrlEncodedBody(("value", "test@example.com"))
