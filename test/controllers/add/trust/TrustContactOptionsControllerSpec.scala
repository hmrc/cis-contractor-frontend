...
class TrustContactOptionsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val trustContactOptionsRoute: String = controllers.add.trust.routes.TrustContactOptionsController.onPageLoad(NormalMode).url
  ...
  "TrustContactOptions Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(TrustContactOptionsPage, TrustContactOptions.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(TrustContactOptions.values.head), NormalMode)(request, messages(application)).toString
      }
    }
