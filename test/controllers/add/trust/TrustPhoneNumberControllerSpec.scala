...
class TrustPhoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val trustPhoneNumberFinalValidationRoute: String =
    controllers.add.trust.routes.TrustPhoneNumberController.onPageLoad(FinalValidationMode).url
  ...
  "TrustPhoneNumberController" - {
    ...
    "must populate the view correctly on a GET when previously answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("0123456723"), NormalMode, trustName)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must redirect to Journey Recovery for a GET when trust name is missing" in {
      ...
      val application = applicationBuilder(userAnswers =
        Some(emptyUserAnswers.set(TrustContactMethodOptionsPage, Set(ContactMethodOptions.Phone)).success.value)
      ).build()
    ...
    "must return OK for a GET in FinalValidationMode when TrustContactMethodOptions is missing" in {
      ...
      running(application) {
        val request = FakeRequest(GET, trustPhoneNumberFinalValidationRoute)
    ...
    "must submit successfully in FinalValidationMode when TrustContactMethodOptions is missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, trustPhoneNumberFinalValidationRoute)
            .withFormUrlEncodedBody(("value", "01632 960 001"))
