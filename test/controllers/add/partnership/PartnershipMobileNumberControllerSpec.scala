...
class PartnershipMobileNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val formProvider = new PartnershipMobileNumberFormProvider()
  private val form         = formProvider()

  private val partnershipName = "Test Name"
  ...
  "PartnershipMobileNumberController" - {
    ...
    "must populate the view correctly on a GET when previously answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("6789432190"), NormalMode, partnershipName)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, partnershipMobileNumberRoute)
    ...
    "must redirect to Journey Recovery for a POST when partnership name is missing (userAnswers present)" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, partnershipMobileNumberRoute)
            .withFormUrlEncodedBody("value" -> "9865234120")
    ...
    "must redirect to contact method option page for a GET when partnership name  is missing" in {

      val application = applicationBuilder(userAnswers =
        Some(emptyUserAnswers.set(PartnershipContactMethodOptionsPage, Set(ContactMethodOptions.Mobile)).success.value)
      ).build()
    ...
    "must redirect to contact method option page for a GET when Mobile is not in PartnershipContactMethodOptions" in {
    ...
    "must redirect to Journey Recovery for a POST when Mobile is not in PartnershipContactMethodOptions" in {
