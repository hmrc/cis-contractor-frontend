...
class PartnershipHasUtrYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val AmendRouteLoad   =
    controllers.add.partnership.routes.PartnershipHasUtrYesNoController.onPageLoad(AmendMode).url
  private lazy val AmendRouteSubmit =
    controllers.add.partnership.routes.PartnershipHasUtrYesNoController.onSubmit(AmendMode).url
  ...
  "partnershipHasUtrYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request = FakeRequest(GET, AmendRouteLoad)
    ...
    "must redirect to the next page when valid data is submitted when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, AmendRouteSubmit)
            .withFormUrlEncodedBody(("value", "true"))
