...
class TradingNameOfSubcontractorControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NameOfSubcontractor Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = uaWithTradingNameOption.set(TradingNameOfSubcontractorPage, "answer").success.value
    ...
    "must return OK and the correct view for a GET when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request = FakeRequest(GET, nameOfSubcontractorAmendRoute)
    ...
    "must redirect to the AmendIndividualCheckYourAnswers page when valid data is submitted when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, nameOfSubcontractorAmendRoute)
            .withFormUrlEncodedBody(("value", "answer"))
    ...
    "must redirect to IndividualNamesOptionsPage for a GET when TradingName is not selected" in {
      ...
      def uaWithSubcontractorNameOption: UserAnswers =
        emptyUserAnswers
