  ...
  "IndividualMobileNumber Controller" - {
    ...
    "must redirect to the next page and not add page to AmendedPagesPage when valid data is submitted in NormalMode" in {
      ...
      running(application) {
        ...
        updatedAnswers.get(IndividualMobileNumberPage) mustBe Some(mobileNumber)
    ...
    "must add IndividualMobileNumberPage to AmendedPagesPage when submitted in AmendMode" in {
      ...
      running(application) {
        ...
        val request =
          FakeRequest(
            POST,
            controllers.add.routes.IndividualMobileNumberController
              .onSubmit(AmendMode)
              .url
        ...
        updatedAnswers.get(IndividualMobileNumberPage) mustBe Some(mobileNumber)
        updatedAnswers.get(AmendedPagesPage).value must contain(IndividualMobileNumberPage.toString)
      }
    }
