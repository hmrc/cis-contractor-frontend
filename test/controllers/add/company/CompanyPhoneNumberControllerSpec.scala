...
class CompanyPhoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CompanyPhoneNumberController" - {
    ...
    "must populate the view correctly on a GET when previously answered" in {
      ...
      val userAnswers =
        uaWithNameAndPhoneOption
          .set(CompanyPhoneNumberPage, "0123456723")
          .success
          .value
    ...
    "must redirect to the CompanyUtrYesNo page and not add the page to AmendedPagesPage when valid data submitted in NormalMode" in {
      ...
      running(application) {
        ...
        updatedAnswers.get(CompanyPhoneNumberPage) mustBe Some("0123456733")
    ...
    "must add the page to AmendedPagesPage when valid data is submitted in AmendMode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, controllers.add.company.routes.CompanyPhoneNumberController.onPageLoad(AmendMode).url)
            .withFormUrlEncodedBody(("value", "0123456733"))
        ...
        updatedAnswers.get(CompanyPhoneNumberPage) mustBe Some("0123456733")
        updatedAnswers.get(AmendedPagesPage) mustBe Some(Set(CompanyPhoneNumberPage.toString))
      }
    }
    ...
    "must redirect to Journey Recovery for a GET when Phone is not in CompanyContactMethodOptions" in {
    ...
    "must redirect to Journey Recovery for a POST when Phone is not in CompanyContactMethodOptions" in {
