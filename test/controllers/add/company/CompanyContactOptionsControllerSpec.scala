...
class CompanyContactOptionsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val companyContactOptionsRoute = controllers.add.company.routes.CompanyContactOptionsController.onPageLoad(NormalMode).url
  ...
  "CompanyContactOptions Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CompanyContactOptionsPage, CompanyContactOptions.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(CompanyContactOptions.values.head), NormalMode)(request, messages(application)).toString
      }
    }
