...
class CompanyUtrYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val companyUtrYesNoAmendRoute =
    controllers.add.company.routes.CompanyUtrYesNoController.onPageLoad(AmendMode).url
  ...
  "CompanyUtrYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request = FakeRequest(GET, companyUtrYesNoAmendRoute)
    ...
    "must redirect to the next page when valid data is submitted when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, companyUtrYesNoAmendRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.add.company.routes.CompanyUtrController
