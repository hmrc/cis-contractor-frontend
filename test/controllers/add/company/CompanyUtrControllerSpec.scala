...
class CompanyUtrControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val formProvider = new CompanyUtrFormProvider()
  ...
  "CompanyUtr Controller" - {
    ...
    "must redirect to yesOrNo page when yesorno page has No for a GET" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(uaWithName.set(CompanyUtrYesNoPage, false).success.value)).build()
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = uaWithName.set(CompanyUtrPage, "7777777777").success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("7777777777"), NormalMode, companyName)(
          request,
    ...
    "must return a Bad Request and show duplicate error when when utr already exists" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(formWithDuplicateError, NormalMode, companyName)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must return OK and the correct view for a GET when subcontractor is unverified in amend mode" in {
      ...
      running(application) {
        val request = FakeRequest(GET, companyUtrRouteAmend)
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form, AmendMode, companyName)(request, messages(application)).toString
      }
    }
