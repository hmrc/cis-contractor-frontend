...
class CompanyCrnControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new CompanyCrnFormProvider()
  ...
  "CompanyCrn Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val application = applicationBuilder(userAnswers = Some(usWithName)).build()
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[CompanyCrnView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = usWithName.set(CompanyCrnPage, "AC012345").success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("AC012345"), NormalMode, companyName)(
          request,
    ...
    "must redirect to the CompanyWorksReferenceYesNoPage page and not add the page to AmendedPagesPage" +
      "when valid data is submitted in NormalMode" in {
        val captor                = ArgumentCaptor.forClass(classOf[UserAnswers])
        val onwardRoute           = controllers.add.company.routes.CompanyWorksReferenceYesNoController.onPageLoad(NormalMode)
        val mockSessionRepository = mock[SessionRepository]

        when(mockSessionRepository.set(any())) thenReturn Future.successful(true)

        val application =
          applicationBuilder(userAnswers = Some(usWithName))
            .overrides(
        ...
        running(application) {
          val request =
            FakeRequest(POST, companyCrnRoute)
          ...
          updatedAnswers.get(CompanyCrnPage) mustBe Some("AC012345")
    ...
    "must add the page to AmendedPagesPage when valid data is submitted in AmendMode" in {
      val captor                = ArgumentCaptor.forClass(classOf[UserAnswers])
      val onwardRoute           = controllers.add.company.routes.CompanyWorksReferenceYesNoController.onPageLoad(AmendMode)
      val mockSessionRepository = mock[SessionRepository]
      ...
      running(application) {
        val request =
          FakeRequest(POST, controllers.add.company.routes.CompanyCrnController.onPageLoad(AmendMode).url)
            .withFormUrlEncodedBody(("value", "AC012345"))
        ...
        updatedAnswers.get(CompanyCrnPage) mustBe Some("AC012345")
        updatedAnswers.get(AmendedPagesPage) mustBe Some(Set(CompanyCrnPage.toString))
      }
    }
