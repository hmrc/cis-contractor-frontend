  ...
  "IndividualEmailAddress Controller" - {
    ...
    "must redirect to the next page and not add page to AmendedPagesPage when valid data is submitted in NormalMode" in {
      ...
      running(application) {
        ...
        updatedAnswers.get(IndividualEmailAddressPage) mustBe Some("abc@xyz.com")
    ...
    "must add TrustEmailAddressPage to AmendedPagesPage when submitted in AmendMode" in {
      ...
      running(application) {
        ...
        val request =
          FakeRequest(
            POST,
            controllers.add.routes.IndividualEmailAddressController
              .onSubmit(AmendMode)
              .url
        ...
        updatedAnswers.get(IndividualEmailAddressPage) mustBe Some("test@example.com")
