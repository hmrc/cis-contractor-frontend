...
import pages.add.{AddIndividualContactMethodsYesNoPage, SubcontractorNamePage}
import pages.amend.AmendedPagesPage
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import repositories.SessionRepository
import views.html.add.AddIndividualContactMethodsYesNoView

import scala.concurrent.Future

class AddIndividualContactMethodsYesNoControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new AddIndividualContactMethodsYesNoFormProvider()
  private val form         = formProvider()

  private lazy val addIndividualContactMethodsYesNoRoute =
  ...
  "AddIndividualContactMethodsYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, addIndividualContactMethodsYesNoRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AddIndividualContactMethodsYesNoView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = uaWithName.set(AddIndividualContactMethodsYesNoPage, true).success.value
    ...
    "must redirect to the AddIndividualContactMethodsYesNo page and not add the page to AmendedPagesPage when" +
      "valid data with value Yes is submitted in NormalMode" in {
      val onwardRoute = controllers.add.routes.IndividualContactMethodOptionsController
      ...
      running(application) {
        val request =
          FakeRequest(POST, addIndividualContactMethodsYesNoRoute)
        ...
        updatedAnswers.get(AddIndividualContactMethodsYesNoPage) mustBe Some(true)
        updatedAnswers.get(AmendedPagesPage) mustBe None
      }
    }

    "must add AddIndividualContactMethodsYesNoPage to AmendedPagesPage when submitted in AmendMode" in {
      val mockSessionRepository = mock[SessionRepository]
      val captor = ArgumentCaptor.forClass(classOf[UserAnswers])
      val onwardRoute = controllers.add.routes.IndividualContactMethodOptionsController.onPageLoad(AmendMode)
      ...
      running(application) {
        ...
        updatedAnswers
          ...
          .value must contain(AddIndividualContactMethodsYesNoPage.toString)
      }
    }
    ...
    "must return a Bad Request and errors when no value is submitted" in {
      ...
      running(application) {
        ...
        val form      = new AddIndividualContactMethodsYesNoFormProvider()()
        ...
        contentAsString(result) must include(messages(application)("addIndividualContactMethodsYesNo.error.required"))
      }
    }
