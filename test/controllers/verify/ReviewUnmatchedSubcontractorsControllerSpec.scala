...
class ReviewUnmatchedSubcontractorsControllerSpec extends SpecBase {
  ...
  private val batch = batchOf(Seq(unmatchedSub, readySub))
  ...
  "ReviewUnmatchedSubcontractorsController" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val service   = application.injector.instanceOf[ReviewUnmatchedSubcontractorsService]
        val view      = application.injector.instanceOf[ReviewUnmatchedSubcontractorsView]
        val viewModel = service.buildViewModel(batch)(messages(application))
    ...
    "must show the unmatched subcontractor, the ready subcontractor and 'None provided'" in {
      ...
      running(application) {
        ...
        body must include(messages(application)("verify.reviewUnmatched.noneProvided"))
      }
    }
    ...
    "must redirect to Journey Recovery when the batch has no unmatched or ready subcontractors" in {

      val userAnswers =
        emptyUserAnswers
          .set(CurrentVerificationBatchResponsePage, batchOf(Nil))
          .success
          .value
