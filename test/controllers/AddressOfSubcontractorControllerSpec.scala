...
class AddressOfSubcontractorControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val addressOfSubcontractorRoute = routes.AddressOfSubcontractorController.onPageLoad(NormalMode).url
  ...
  "AddressOfSubcontractor Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(AddressOfSubcontractor("value 1", "value 2")), NormalMode)(request, messages(application)).toString
      }
    }
