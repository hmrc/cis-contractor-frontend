...
class CompanyCheckYourAnswersControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val insufficientRouteUrl = controllers.info.company.routes.CompanyCheckYourAnswersController
    .onPageLoad(journeyType)
    .url

  "CompanyCheckYourAnswersController" - {

    "must return OK and render the correct summary for an unverified company" in {

      val userAnswers =
        emptyUserAnswers
          .set(CompanyAnswersQuery, answers)
          .success
          .value
    ...
    "must render the correct summary for a verified company" in {
      ...
      val userAnswers =
        emptyUserAnswers
          .set(CompanyAnswersQuery, verifiedAnswers)
          .success
          .value
    ...
    "must not render the verification number row when the company is pending verification" in {
      ...
      val userAnswers =
        emptyUserAnswers
          .set(CompanyAnswersQuery, pendingAnswers)
          .success
          .value
    ...
    "must render the correct back to message and Url when the journey is unmatched" in {
      ...
      lazy val unmatchedRouteUrl = controllers.info.company.routes.CompanyCheckYourAnswersController
        .onPageLoad(journeyType)
        .url
    ...
    "must redirect to Journey Recovery when when journeyType is not valid" in {
      ...
      lazy val invalidRouteUrl = controllers.info.company.routes.CompanyCheckYourAnswersController
        .onPageLoad(journeyType)
        .url
