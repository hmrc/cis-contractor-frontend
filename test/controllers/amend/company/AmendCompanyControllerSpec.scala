...
class AmendCompanyControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AmendCompanyController" - {
    ...
    "onPageLoad" - {
      ...
      "must redirect to JourneyRecovery and not save when the subcontractor is not a company" in {
        val mockService           = mock[SubcontractorService]
        val mockSessionRepository = mock[SessionRepository]
      ...
      "must show verification details when the company is verified" in {
        val mockService           = mock[SubcontractorService]
        val mockSessionRepository = mock[SessionRepository]
        val captor                =
          ArgumentCaptor.forClass(classOf[UserAnswers])
        ...
        when(
          ...
          Future.successful(
            response.copy(
              subcontractor = Some(verifiedCompany)
            )
          )
        )
      ...
      "must show verification details when the company is unverified but has a pending verification" in {
        val mockService           = mock[SubcontractorService]
        val mockSessionRepository = mock[SessionRepository]
        val captor                =
          ArgumentCaptor.forClass(classOf[UserAnswers])
        ...
        when(
          ...
          Future.successful(
            response.copy(
              subcontractor = Some(pendingCompany)
            )
          )
        )
      ...
      "must hide verification details when the company is unverified and has no pending verification" in {
        val mockService           = mock[SubcontractorService]
        val mockSessionRepository = mock[SessionRepository]
        val captor                =
          ArgumentCaptor.forClass(classOf[UserAnswers])

        val unverifiedCompany =
          subcontractor.copy(
            verified = Some("N"),
            pendingVerifications = Some(0)
          )
        ...
        when(
          ...
          Future.successful(
            response.copy(
              subcontractor = Some(unverifiedCompany)
            )
          )
        )
