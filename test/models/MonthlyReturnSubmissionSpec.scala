...
final class MonthlyReturnSubmissionSpec extends AnyWordSpec with Matchers {
  "MonthlyReturnSubmission JSON format" should {
    ...
    "read JSON into model (including missing optional fields)" in {
      ...
      val result = json.validate[MonthlyReturnSubmission]
    ...
    "write model to JSON" in {
      val model = MonthlyReturnSubmission(
        submissionId = 555L,
    ...
    "round-trip (model -> json -> model) without losing data" in {
      ...
      val json = Json.toJson(model)
      json.validate[MonthlyReturnSubmission] mustBe JsSuccess(model)
    }
  }
}
