...
class PartnershipContactMethodOptionsSpec
    extends AnyFreeSpec
    with Matchers
    ...
    "must deserialise valid values" in {
      ...
      forAll(gen) { partnershipContactMethodOptions =>
        JsString(partnershipContactMethodOptions.toString)
          .validate[PartnershipContactMethodOptions]
          .asOpt
          .value mustEqual partnershipContactMethodOptions
      }
    }

    "must fail to deserialise invalid values" in {

      val gen = arbitrary[String] suchThat (!PartnershipContactMethodOptions.values.map(_.toString).contains(_))

      forAll(gen) { invalidValue =>
        JsString(invalidValue).validate[PartnershipContactMethodOptions] mustEqual JsError("error.invalid")
      }
    }
