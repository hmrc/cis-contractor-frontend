...
class AmendUniqueTaxpayerReferenceYesNoSummarySpec extends AnyFreeSpec with Matchers {

  implicit val messages: Messages = stubMessages()

  "AmendUniqueTaxpayerReferenceYesNoSummary.row" - {
    ...
    "must return a SummaryListRow with 'Yes' when the answer is true" in {
      ...
      val maybeRow: Option[SummaryListRow] = AmendUniqueTaxpayerReferenceYesNoSummary.row(answers)
      ...
      changeAction.attributes should contain("id" -> "unique-taxpayer-reference-yes-no")
    }
    ...
    "must return None when the answer does not exist" in {
      val answers = UserAnswers("test-id")
      AmendUniqueTaxpayerReferenceYesNoSummary.row(answers) shouldBe None
    }
  }
}
