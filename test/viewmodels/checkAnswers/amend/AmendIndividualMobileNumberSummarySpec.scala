...
class AmendIndividualMobileNumberSummarySpec extends AnyFreeSpec with Matchers with CyaEncodingSpecHelper {
  implicit val messages: Messages = stubMessages()

  "AmendIndividualMobileNumberSummary.row" - {
    ...
    "must return a Summary List Row when the answer exists" in {
      ...
      val maybeRow = AmendIndividualMobileNumberSummary.row(answers)
      ...
      val expectedChangeText = messages("site.update")
    ...
    "must return None when the answer does not exist" in {
      ...
      AmendIndividualMobileNumberSummary.row(answers) shouldBe None
    }
    ...
    "must HTML-escape special characters correctly (single encoding only)" in {
      ...
      val answers =
        UserAnswers("id")
          .set(IndividualMobileNumberPage, mobile)
          .success
          .value

      val row = AmendIndividualMobileNumberSummary.row(answers).value
