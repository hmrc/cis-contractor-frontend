...
class AmendWorksReferenceNumberYesNoSummarySpec extends AnyFreeSpec with Matchers {

  implicit val messages: Messages = stubMessages()

  "AmendWorksReferenceNumberYesNoSummary.row" - {
    ...
    "must return a SummaryListRow with 'Yes' when the answer is true" in {
      ...
      val maybeRow: Option[SummaryListRow] = AmendWorksReferenceNumberYesNoSummary.row(answers)
      ...
      changeAction.attributes                   must contain("id" -> "works-reference-number-yes-no")
    }
    ...
    "must return None when the answer does not exist" in {
      val answers = UserAnswers("test-id")
      AmendWorksReferenceNumberYesNoSummary.row(answers) shouldBe None
    }
  }
}
