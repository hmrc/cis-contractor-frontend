...
class AmendSubTradingNameYesNoSummarySpec extends AnyFreeSpec with Matchers {
  implicit val messages: Messages = stubMessages()

  "AmendSubTradingNameYesNoSummary.row" - {
    ...
    "must return a SummaryListRow with 'Yes' when the answer is true" in {
      ...
      val maybeRow: Option[SummaryListRow] = AmendSubTradingNameYesNoSummary.row(answers)
    ...
    "must return None when the answer does not exist" in {
      val answers = UserAnswers("test-id")
      AmendSubTradingNameYesNoSummary.row(answers) shouldBe None
    }
  }
}
