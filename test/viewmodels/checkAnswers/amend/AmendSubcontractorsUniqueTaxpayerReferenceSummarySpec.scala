...
class AmendSubcontractorsUniqueTaxpayerReferenceSummarySpec
    extends AnyFreeSpec
    with Matchers
  ...
  "AmendSubcontractorsUniqueTaxpayerReferenceSummary.row" - {
    ...
    "must return a SummaryListRow when the answer exists" in {
      ...
      val maybeRow = AmendSubcontractorsUniqueTaxpayerReferenceSummary.row(answers)
      ...
      changeAction.attributes                 should contain("id" -> "subcontractors-unique-taxpayer-reference")
    }

    "must return None when the answer does not exist" in {
      val answers = UserAnswers("test-id")
      AmendSubcontractorsUniqueTaxpayerReferenceSummary.row(answers) shouldBe None
    }
    ...
    "must HTML-escape special characters correctly (single encoding only)" in {
      ...
      val row = AmendSubcontractorsUniqueTaxpayerReferenceSummary.row(answers).value
