...
class AmendIndividualEmailAddressSummarySpec extends AnyFreeSpec with Matchers with CyaEncodingSpecHelper {

  implicit val messages: Messages = stubMessages()

  "AmendIndividualEmailAddressSummary.row" - {

    "must return a SummaryListRow when the answer exists" in {

      val answers =
        UserAnswers("test-id")
          .set(IndividualEmailAddressPage, "test@example.com")
          .success
          .value

      val maybeRow = AmendIndividualEmailAddressSummary.row(answers)

      maybeRow shouldBe defined

      val row = maybeRow.value

      val expectedKeyText = messages("individualEmailAddress.checkYourAnswersLabel")
    ...
    "must return None when the answer does not exist" in {
      ...
      AmendIndividualEmailAddressSummary.row(answers) shouldBe None
    }
    ...
    "must HTML-escape special characters correctly (single encoding only)" in {
      ...
      val row = AmendIndividualEmailAddressSummary.row(answers).value
