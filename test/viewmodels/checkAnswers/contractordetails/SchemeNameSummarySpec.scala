...
class SchemeNameSummarySpec extends AnyFreeSpec with Matchers {

  implicit val messages: Messages = stubMessages()

  "SchemeNameSummary.row" - {

    "must return a Summary List Row when the answer exists" in {
      val answers =
        UserAnswers("test-id")
          .set(SchemeNamePage, "My Construction Scheme")
          .success
          .value
      ...
      val expectedKeyText = messages("contractordetails.schemeName.checkYourAnswersLabel")
      row.key.content.asHtml.toString should include(expectedKeyText)

      row.value.content.asHtml.toString should include("My Construction Scheme")
