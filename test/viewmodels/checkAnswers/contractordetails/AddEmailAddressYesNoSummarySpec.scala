...
class AddEmailAddressYesNoSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "AddEmailAddressYesNoSummary.row" - {
    ...
    "must return a SummaryListRow with 'Yes' when the answer is true" in {
      ...
      val maybeRow: Option[SummaryListRow] = AddEmailAddressYesNoSummary.row(answers)
      maybeRow shouldBe defined

      val row =
        maybeRow.value

      val expectedKeyText = messages("contractordetails.addEmailAddressYesNo.checkYourAnswersLabel")
      ...
      val expectedHref       = controllers.contractordetails.routes.AddEmailAddressYesNoController.onPageLoad(CheckMode).url
      val expectedHiddenText = messages("contractordetails.addEmailAddressYesNo.change.hidden")
