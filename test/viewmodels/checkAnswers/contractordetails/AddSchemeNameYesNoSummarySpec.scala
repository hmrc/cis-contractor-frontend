...
class AddSchemeNameYesNoSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "AddSchemeNameYesNoSummary.row" - {
    ...
    "must return a SummaryListRow with 'Yes' when the answer is true" in {
      ...
      val maybeRow: Option[SummaryListRow] = AddSchemeNameYesNoSummary.row(answers)
      maybeRow shouldBe defined

      val row =
        maybeRow.value

      val expectedKeyText = messages("contractordetails.addSchemeNameYesNo.checkYourAnswersLabel")
      ...
      val expectedHref       = controllers.contractordetails.routes.AddSchemeNameYesNoController.onPageLoad(CheckMode).url
      val expectedHiddenText = messages("contractordetails.addSchemeNameYesNo.change.hidden")
    ...
    "must link Change to the remove scheme name page when a scheme UTR already exists" in {
      val scheme = Scheme(
        schemeId = 1,
        instanceId = "cisId",
