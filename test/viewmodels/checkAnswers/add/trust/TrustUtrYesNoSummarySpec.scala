...
class TrustUtrYesNoSummarySpec extends SpecBase with GuiceOneAppPerSuite {
  ...
  "TrustUtrYesNoSummary.row" - {

    "return a row with correct key, value = yes, and change action when the answer is true" in {
      val ua: UserAnswers = emptyUserAnswers.set(TrustUtrYesNoPage, true).success.value

      val maybeRow = TrustUtrYesNoSummary.row(ua)
    ...
    "return a row with correct key, value = no, and change action when the answer is false" in {
