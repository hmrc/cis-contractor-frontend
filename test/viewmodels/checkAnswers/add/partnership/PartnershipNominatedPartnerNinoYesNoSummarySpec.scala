...
class PartnershipNominatedPartnerNinoYesNoSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "PartnershipNominatedPartnerNinoYesNoSummary.row" - {
    ...
    "return a row with key, value = no, and change action pointing to add flow when the answer is false in AmendMode" in {
      ...
      val maybeRow: Option[SummaryListRow] = PartnershipNominatedPartnerNinoYesNoSummary.row(ua, AmendMode)
      maybeRow shouldBe defined

      val row: SummaryListRow = maybeRow.value

      val expectedKeyText = messages("partnershipNominatedPartnerNinoYesNo.checkYourAnswersLabel")
      row.key.content.asHtml.toString should include(expectedKeyText)
      ...
      val changeAction       = actions.head
      val expectedHref       = controllers.add.partnership.routes.PartnershipNominatedPartnerNinoYesNoController
        .onPageLoad(AmendMode)
        .url
      val expectedChangeText = messages("site.change")
      val expectedHiddenText = messages("partnershipNominatedPartnerNinoYesNo.change.hidden")

      changeAction.content.asHtml.toString    should include(expectedChangeText)
      changeAction.href                     shouldBe expectedHref
      changeAction.visuallyHiddenText.value shouldBe expectedHiddenText
      changeAction.attributes                 should contain("id" -> "add-nominated-partner-nino")
    }
