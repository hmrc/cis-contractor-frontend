...
class PartnershipNominatedPartnerNameSummarySpec extends AnyFreeSpec with Matchers with CyaEncodingSpecHelper {
  ...
  "PartnershipNominatedPartnerNameSummary.row" - {
    ...
    "must return a SummaryListRow when the answer exists in Amend journey" in {
      ...
      val maybeRow = PartnershipNominatedPartnerNameSummary.row(answers, AmendMode)
      maybeRow shouldBe defined
      ...
      val changeAction       = actions.head
      val expectedChangeText = messages("site.change")
      val expectedHref       = routes.PartnershipNominatedPartnerNameController.onPageLoad(AmendMode).url
      val expectedHiddenText = messages("partnershipNominatedPartnerName.change.hidden")
    ...
    "must HTML-escape special characters correctly (single encoding only)" in {
      ...
      val name = "O'Reilly & Sons"

      val answers =
        UserAnswers("id")
          .set(PartnershipNominatedPartnerNamePage, name)
          .success
          .value

      val row = PartnershipNominatedPartnerNameSummary.row(answers).value
