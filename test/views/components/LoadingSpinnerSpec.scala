...
class LoadingSpinnerSpec extends SpecBase with Matchers {
  ...
  "LoadingSpinner" - {
    ...
    "must render the page with correct heading, paragraphs, and other contents" in new Setup {
      val headingText: String         = testHeading
      val paragraphText: String       = testText
      val html: HtmlFormat.Appendable = loadingSpinner(headingText, paragraphText)
      val doc: Document               = Jsoup.parse(html.body)

      val heading: Elements = getHeadingElement(html)
      heading.size mustBe 1
      heading.text mustBe headingText

      val para: Elements = getParagraphElement(html)
      para.size mustBe 1
  ...
  trait Setup {
    ...
    val testHeading = "Test heading content"
