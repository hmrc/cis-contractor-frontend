...
import forms.contractordetails.AddEmailAddressYesNoFormProvider
import models.NormalMode
import org.jsoup.Jsoup
import org.jsoup.nodes.Document
import play.api.i18n.Messages
import play.api.test.FakeRequest
import views.html.contractordetails.AddEmailAddressYesNoView
...
class AddEmailAddressYesNoViewSpec extends SpecBase {
  ...
  "AddEmailAddressYesNoView" - {
    "must render the page with the correct html elements" in new Setup {
      ...
      doc.title             must include(messages("contractordetails.addEmailAddressYesNo.title"))
      doc.select("h1").text must include(messages("contractordetails.addEmailAddressYesNo.heading"))
