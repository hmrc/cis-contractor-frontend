...
class TrustEmailAddressViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "TrustEmailAddressView" should {
    ...
    "render the page with title, heading, input and submit button" in new Setup {
      ...
      heading.text() mustBe messages("trustEmailAddress.heading", trustName)

      doc.select("form").attr("action") mustBe controllers.add.trust.routes.TrustEmailAddressController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      ...
      doc.select(".govuk-error-message").text() must include(messages("trustEmailAddress.error.required"))
    }
  }
  ...
  trait Setup {
    ...
    val view: TrustEmailAddressView = app.injector.instanceOf[TrustEmailAddressView]
  }
}
