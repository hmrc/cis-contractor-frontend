...
class TrustUtrYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustUtrYesNoView" should {
    ...
    "render the page with title, heading, hint, yes/no radios and submit button" in new Setup {
      ...
      doc.select("title").text() must include(
        messages("trustUtrYesNo.title")
      )

      val legend = doc.select("fieldset legend")
      legend.text() mustBe messages("trustUtrYesNo.heading", trustName)
      legend.hasClass("govuk-fieldset__legend--l") mustBe true

      doc.select(".govuk-hint").text() mustBe messages("trustUtrYesNo.hint")
  ...
  trait Setup {
    ...
    val view: TrustUtrYesNoView =
      app.injector.instanceOf[TrustUtrYesNoView]
  }
}
