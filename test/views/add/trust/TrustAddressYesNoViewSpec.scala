...
import views.html.add.trust.TrustAddressYesNoView

class TrustAddressYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustAddressYesNoView" should {

    "render the page with title, heading, hint, yes/no radios and submit button" in new Setup {

      val trustName = "Test trustName"

      val html = view(form, NormalMode, trustName)
      ...
      doc.select(".govuk-hint").text() mustBe messages("trustAddressYesNo.hint")
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      ...
      val errorForm = form.withError("value", "trustAddressYesNo.error.required")
  ...
  trait Setup {
    ...
    val view: TrustAddressYesNoView =
      app.injector.instanceOf[TrustAddressYesNoView]
  }
}
