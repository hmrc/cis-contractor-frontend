...
class TrustNameViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustNameView" should {
    ...
    "render the page with title, heading, input and submit button" in new Setup {
      ...
      doc.select("title").text() must include(messages("trustName.title"))

      val heading = doc.select("label.govuk-label")
      heading.text() mustBe messages("trustName.heading")

      doc.select("form").attr("action") mustBe controllers.add.trust.routes.TrustNameController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      val errorForm: Form[String] =
        form.withError("value", "trustName.error.required")

      val html = view(errorForm, NormalMode)
      val doc  = org.jsoup.Jsoup.parse(html.toString())

      val summary = doc.select(".govuk-error-summary")
      summary.text() must include(messages("trustName.error.required"))

      val linkHref = summary.select("a").attr("href")
      linkHref mustBe "#value"

      doc.select(".govuk-error-message").text() must include(messages("trustName.error.required"))
    }
  }
  ...
  trait Setup {
    ...
    val view: TrustNameView = app.injector.instanceOf[TrustNameView]
  }
}
