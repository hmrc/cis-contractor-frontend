...
class TrustWorksReferenceYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustWorksReferenceYesNoView" should {
    ...
    "render the page with title, heading, hint, radios and submit button" in new Setup {
      ...
      doc
        .select("form")
        .attr("action") mustBe controllers.add.trust.routes.TrustWorksReferenceYesNoController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      ...
      val errorForm: Form[Boolean] =
        form.withError("value", "trustWorksReferenceYesNo.error.required")
  ...
  trait Setup {
    ...
    val view: TrustWorksReferenceYesNoView =
      app.injector.instanceOf[TrustWorksReferenceYesNoView]
  }
}
