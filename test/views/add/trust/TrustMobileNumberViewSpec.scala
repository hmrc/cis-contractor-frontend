...
class TrustMobileNumberViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustMobileNumberView" should {
    ...
    "render the page with title, heading, input and submit button" in new Setup {
      ...
      heading.text() mustBe messages("trustMobileNumber.heading", trustName)

      val hint = doc.select(".govuk-hint")
      hint.text() mustBe messages("trustMobileNumber.hint")

      doc
        .select("form")
        .attr("action") mustBe controllers.add.trust.routes.TrustMobileNumberController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      ...
      val errorForm: Form[String] =
        form.withError("value", "trustMobileNumber.error.required")

      val html = view(errorForm, NormalMode, trustName)
      val doc  = org.jsoup.Jsoup.parse(html.toString())

      val summary = doc.select(".govuk-error-summary")
      summary.text() must include(messages("trustMobileNumber.error.required"))

      val linkHref = summary.select("a").attr("href")
      linkHref mustBe "#value"

      doc.select(".govuk-error-message").text() must include(messages("trustMobileNumber.error.required"))
    }
  }
  ...
  trait Setup {
    ...
    val view: TrustMobileNumberView = app.injector.instanceOf[TrustMobileNumberView]
  }
}