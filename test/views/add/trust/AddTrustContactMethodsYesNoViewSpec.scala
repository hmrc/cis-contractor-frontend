...
class AddTrustContactMethodsYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  "AddTrustContactMethodsYesNoView" should {
    ...
    "render the page with title, heading, radios and submit button" in new Setup {
      ...
      doc.select("title").text() must include(messages("addTrustContactMethodsYesNo.title"))

      val legend: Elements = doc.select("fieldset legend")
      legend.text() mustBe messages("addTrustContactMethodsYesNo.heading", trustName)
      ...
      doc
        .select("form")
        .attr("action") mustBe controllers.add.trust.routes.AddTrustContactMethodsYesNoController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      ...
      val errorForm: Form[Boolean] =
        form.withError("value", "addTrustContactMethodsYesNo.error.required")

      val html: HtmlFormat.Appendable = view(errorForm, NormalMode, trustName)
      val doc: Document               = org.jsoup.Jsoup.parse(html.toString())

      val summary: Elements = doc.select(".govuk-error-summary")
      summary.text() must include(messages("addTrustContactMethodsYesNo.error.required"))

      val linkHref: String = summary.select("a").attr("href")
      linkHref mustBe "#value_0"

      doc.select(".govuk-error-message").text() must include(
        messages("addTrustContactMethodsYesNo.error.required")
      )
    }
  }

  trait Setup {
    val formProvider        = new AddTrustContactMethodsYesNoFormProvider()
    ...
    val view: AddTrustContactMethodsYesNoView = app.injector.instanceOf[AddTrustContactMethodsYesNoView]
  }
}
