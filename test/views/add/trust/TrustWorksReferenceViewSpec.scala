...
class TrustWorksReferenceViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "TrustWorksReferenceView" should {
    ...
    "render the page with title, heading, input and submit button" in new Setup {
      ...
      heading.text() mustBe messages("trustWorksReference.heading", trustName)

      doc.select("form").attr("action") mustBe controllers.add.trust.routes.TrustWorksReferenceController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      val errorForm: Form[String] =
        form.withError("value", "trustWorksReference.error.required")
      ...
      summary.text() must include(messages("trustWorksReference.error.required"))

      val linkHref = summary.select("a").attr("href")
      linkHref mustBe "#value"

      doc.select(".govuk-error-message").text() must include(messages("trustWorksReference.error.required"))
    }
  }

  trait Setup {
    val formProvider       = new TrustWorksReferenceFormProvider()
    ...
    val view: TrustWorksReferenceView = app.injector.instanceOf[TrustWorksReferenceView]
  }
