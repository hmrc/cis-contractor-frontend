...
class CompanyCrnYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  private val companyName = "Test Company Name"

  "CompanyCrnYesNoView" should {
    ...
    "render the page with title, heading, radios and submit button" in new Setup {
      ...
      doc.select("title").text() must include(messages("companyCrnYesNo.title"))

      val legend: Elements = doc.select("fieldset legend")
      legend.text() mustBe messages("companyCrnYesNo.heading", companyName)
      legend.hasClass("govuk-fieldset__legend--l") mustBe true

      val hint: Elements = doc.select("fieldset .govuk-hint")
      hint.text() mustBe messages("companyCrnYesNo.hint")
      ...
      doc
        .select("form")
        .attr("action") mustBe controllers.add.company.routes.CompanyCrnYesNoController
        .onSubmit(NormalMode)
        .url
  ...
  trait Setup {
    ...
    val view: CompanyCrnYesNoView =
      app.injector.instanceOf[CompanyCrnYesNoView]
  }
