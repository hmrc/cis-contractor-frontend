...
class CompanyAddressYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "CompanyAddressYesNoView" should {
    ...
    "render the page with title, heading, hint, yes/no radios and submit button" in new Setup {
      ...
      doc.select(".govuk-hint").text() mustBe messages("companyAddressYesNo.hint")
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      ...
      val errorForm = form.withError("value", "companyAddressYesNo.error.required")
  ...
  trait Setup {
    ...
    val view: CompanyAddressYesNoView =
      app.injector.instanceOf[CompanyAddressYesNoView]
  }
}
