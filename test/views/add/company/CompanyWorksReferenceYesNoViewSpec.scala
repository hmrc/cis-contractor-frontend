...
class CompanyWorksReferenceYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "CompanyWorksReferenceYesNoView" should {
    ...
    "render the page with title, heading, hint, radios and submit button" in new Setup {
      ...
      doc
        .select("form")
        .attr("action") mustBe controllers.add.company.routes.CompanyWorksReferenceYesNoController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      ...
      val errorForm: Form[Boolean] =
        form.withError("value", "companyWorksReferenceYesNo.error.required")
      ...
      summary.text() must include(
        messages("companyWorksReferenceYesNo.error.required")
      )
  ...
  trait Setup {
    ...
    val view: CompanyWorksReferenceYesNoView =
      app.injector.instanceOf[CompanyWorksReferenceYesNoView]
  }
}
