...
import views.html.add.SubcontractorContactDetailsYesNoView

class SubcontractorContactDetailsYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "SubcontractorContactDetailsYesNoView" should {
    ...
    "render the page with title, heading, hint, yes/no radios and submit button" in new Setup {
      ...
      doc.select("title").text() must include(messages("subcontractorContactDetailsYesNo.title"))

      val legend = doc.select("fieldset legend")
      legend.text() mustBe messages("subcontractorContactDetailsYesNo.heading")
      legend.hasClass("govuk-fieldset__legend--l") mustBe true

      doc.select(".govuk-hint").text() mustBe messages("subcontractorContactDetailsYesNo.hint")
      ...
      doc.select("form").attr("action") mustBe
        controllers.add.routes.SubcontractorContactDetailsYesNoController.onSubmit(NormalMode).url
  ...
  trait Setup {
    ...
    val view: SubcontractorContactDetailsYesNoView =
      app.injector.instanceOf[SubcontractorContactDetailsYesNoView]
  }
}