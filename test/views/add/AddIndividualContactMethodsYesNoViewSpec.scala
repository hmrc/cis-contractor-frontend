...
class AddIndividualContactMethodsYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  "AddIndividualContactMethodsYesNoView" should {
    ...
    "render the page with title, heading, radios and submit button" in new Setup {
      ...
      legend.text() mustBe messages("addIndividualContactMethodsYesNo.heading", subcontractorName)
      ...
      doc.select("form").attr("action") mustBe controllers.add.routes.AddIndividualContactMethodsYesNoController
        .onSubmit(NormalMode)
        .url
  ...
  trait Setup {
    ...
    val view: AddIndividualContactMethodsYesNoView = app.injector.instanceOf[AddIndividualContactMethodsYesNoView]
  }
}
