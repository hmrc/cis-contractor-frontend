...
class SubcontractorsUniqueTaxpayerReferenceViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "SubcontractorsUniqueTaxpayerReferenceView" should {
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      ...
      doc.select(".govuk-error-message").text() must include(
        messages("subcontractorsUniqueTaxpayerReference.error.required")
      )
    }
  }
  ...
  trait Setup {
    ...
    val view: SubcontractorsUniqueTaxpayerReferenceView =
      app.injector.instanceOf[SubcontractorsUniqueTaxpayerReferenceView]
  }
}
