...
class PartnershipHasUtrYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "PartnershipHasUtrYesNoView" should {
    ...
    "render the page with title, heading, hint, yes/no radios and submit button" in new Setup {
      ...
      legend.text() mustBe messages("partnershipHasUtrYesNo.heading", partnershipName)
      legend.hasClass("govuk-fieldset__legend--l") mustBe true

      doc.select(".govuk-hint").text() mustBe messages("partnershipHasUtrYesNo.hint")
  ...
  trait Setup {
    val formProvider        = new PartnershipHasUtrYesNoFormProvider()
    ...
    val view: PartnershipHasUtrYesNoView =
      app.injector.instanceOf[PartnershipHasUtrYesNoView]
  }
}
