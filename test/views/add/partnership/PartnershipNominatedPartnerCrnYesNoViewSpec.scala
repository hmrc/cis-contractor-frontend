...
class PartnershipNominatedPartnerCrnYesNoViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "PartnershipNominatedPartnerCrnYesNoView" should {
    ...
    "render the page with title, heading, radios and submit button" in new Setup {
      ...
      doc.select("title").text() must include(messages("partnershipNominatedPartnerCrnYesNo.title"))

      val legend: Elements = doc.select("fieldset legend")
      legend.text() mustBe messages("partnershipNominatedPartnerCrnYesNo.heading", nominatedPartnershipName)
      legend.hasClass("govuk-fieldset__legend--l") mustBe true

      val hint: Elements = doc.select("fieldset .govuk-hint")
      hint.text() mustBe messages("partnershipNominatedPartnerCrnYesNo.hint")
      ...
      doc
        .select("form")
        .attr("action") mustBe controllers.add.partnership.routes.PartnershipNominatedPartnerCrnYesNoController
        .onSubmit(NormalMode)
        .url
    ...
    "display error summary and inline error when no option is selected" in new Setup {
      val errorForm: Form[Boolean] =
        form.withError("value", "partnershipNominatedPartnerCrnYesNo.error.required")
  ...
  trait Setup {
    val formProvider        = new PartnershipNominatedPartnerCrnYesNoFormProvider()
    ...
    val view: PartnershipNominatedPartnerCrnYesNoView =
      app.injector.instanceOf[PartnershipNominatedPartnerCrnYesNoView]
  }
