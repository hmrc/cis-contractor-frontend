...
class IndividualMobileNumberViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {

  "IndividualMobileNumberView" should {
    ...
    "display error summary and inline error when no name is entered" in new Setup {
      ...
      val errorForm: Form[String] =
        form.withError("value", "individualMobileNumber.error.required")

      val html = view(errorForm, NormalMode, subcontractorName)
      val doc  = org.jsoup.Jsoup.parse(html.toString())

      val summary = doc.select(".govuk-error-summary")
      summary.text() must include(messages("individualMobileNumber.error.required"))

      val linkHref = summary.select("a").attr("href")
      linkHref mustBe "#value"

      doc.select(".govuk-error-message").text() must include(messages("individualMobileNumber.error.required"))
    }
  }
  ...
  trait Setup {
    ...
    val view: IndividualMobileNumberView = app.injector.instanceOf[IndividualMobileNumberView]
  }
}
