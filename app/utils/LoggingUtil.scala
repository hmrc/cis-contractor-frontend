...
trait LoggingUtil extends LoggerLike {
  ...
  lazy val trueClientIpFromHttpResponse: HttpResponse => Option[String] = httpResponse =>
    httpResponse.headers
      .get(HeaderNames.trueClientIp)
      .flatMap(_.headOption)
      .map(trueClientIp => s"trueClientIp: $trueClientIp ")
  lazy val sessionIdFromHttpResponse: HttpResponse => Option[String]    = httpResponse =>
    httpResponse.headers.get(HeaderNames.xSessionId).flatMap(_.headOption).map(sessionId => s"sessionId: $sessionId ")
  lazy val identifiersFromHttpResponse: HttpResponse => String          = request =>
    Seq(trueClientIpFromHttpResponse(request), sessionIdFromHttpResponse(request)).flatten.foldLeft("")(_ + _)
  ...
  def infoConnectorLog(message: => String)(implicit httpResponse: HttpResponse): Unit =
    this.info(s"$message (${identifiersFromHttpResponse(httpResponse)})")
  ...
  def warnConnectorLog(message: => String)(implicit mc: MarkerContext, httpResponse: HttpResponse): Unit =
    this.warn(s"$message (${identifiersFromHttpResponse(httpResponse)})")
  ...
  def errorConnectorLog(message: => String)(implicit httpResponse: HttpResponse): Unit =
    this.error(s"$message (${identifiersFromHttpResponse(httpResponse)})")
}
