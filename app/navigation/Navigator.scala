...
class Navigator @Inject() () {
  ...
  private val normalRoutes: Page => UserAnswers => Call = {
    ...
    case UniqueTaxpayerReferencePage => userAnswers => navigatorFromUniqueTaxpayerReferencePage(NormalMode)(userAnswers)
  ...
  private val checkRouteMap: Page => UserAnswers => Call = {
    ...
    case UniqueTaxpayerReferencePage => userAnswers => navigatorFromUniqueTaxpayerReferencePage(CheckMode)(userAnswers)
  ...
  private def navigatorFromUniqueTaxpayerReferencePage(mode: Mode)(userAnswers: UserAnswers): Call =
    (userAnswers.get(UniqueTaxpayerReferencePage), mode) match {
      case (Some(true), _) => controllers.add.routes.TradingNameOfSubcontractorController.onPageLoad(mode)
      case (Some(false), NormalMode) => controllers.add.routes.UniqueTaxpayerReferenceController.onPageLoad(NormalMode)
      case (Some(false), CheckMode) => routes.CheckYourAnswersController.onPageLoad()
      case (None, _) => routes.JourneyRecoveryController.onPageLoad()
    }
}
