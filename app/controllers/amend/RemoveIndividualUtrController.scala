...
class RemoveIndividualUtrController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    val result = for {
      a <- request.userAnswers.remove(SubcontractorsUniqueTaxpayerReferencePage)
      b <- a.set(UniqueTaxpayerReferenceYesNoPage, false)
    } yield b
