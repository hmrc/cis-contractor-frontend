...
class ViewOnlyCompanyCheckYourAnswersController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] =
    (identify andThen getData andThen requireData) { implicit request =>
      request.userAnswers.get(ViewOnlyCompanyAnswersQuery) match {
        ...
        case None =>
          logger.error(
            "[ViewOnlyCompanyCheckYourAnswersController.onPageLoad] " +
              "ViewOnlyCompanyAnswersQuery is missing"
          )
