...
class WorksReferenceNumberController @Inject() (
  ...
  private def preparedForm(implicit request: DataRequest[?]) =
    request.userAnswers.get(WorksReferenceNumberPage).fold(form)(form.fill)
            ...
            .fold(
              ...
              value =>
                for {
                  updatedAnswers <-
                    Future.fromTry(
                      SaveAnswerHelper.saveAnswer(request.userAnswers, WorksReferenceNumberPage, value, mode)
                    )
