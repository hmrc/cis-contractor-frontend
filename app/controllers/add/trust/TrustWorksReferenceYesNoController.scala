...
class TrustWorksReferenceYesNoController @Inject() (
  ...
  def onPageLoad(mode: Mode, subbieResourceRef: Long): Action[AnyContent] =
    (identify andThen getData andThen requireData andThen redirectUnmatchSubbieRefActionFilter(
      mode,
      subbieResourceRef
    )) { implicit request =>
      request.userAnswers
        .get(TrustNamePage)
        .map { trustName =>
          val preparedForm = request.userAnswers.get(TrustWorksReferenceYesNoPage) match {
            case None        => form
