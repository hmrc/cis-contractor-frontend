...
import models.finalvalidation.{CreateFinalValidationDraftRequest, FinalValidationCorrection, FinalValidationDraft, FinalValidationDraftIssue, UpdateFinalValidationCorrectionRequest, UpdateFinalValidationReadinessRequest}
...
class FinalValidationDraftService @Inject() (
  ...
  def updateCorrection(
    instanceId: String,
    draftId: String,
    correction: FinalValidationCorrection
  )(implicit hc: HeaderCarrier): Future[FinalValidationDraft] =
    connector.updateFinalValidationCorrection(
      instanceId,
      draftId,
      UpdateFinalValidationCorrectionRequest(
        subcontractorId = correction.subcontractorId,
        changeTarget = correction.changeTarget.key,
