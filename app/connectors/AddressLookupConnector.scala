...
@Singleton
class AddressLookupConnector @Inject() (val http: HttpClientV2, appConfig: FrontendAppConfig)(implicit
  ec: ExecutionContext
) extends LoggingUtil {
  ...
  def getAddress(id: String)(implicit hc: HeaderCarrier): Future[Address] =
    http
      .get(url"${appConfig.addressLookupRetrievalUrl(id)}")
      .execute[Address]

  def getOnRampUrl(
    alfConfig: AddressLookupConfigurationModel
  )(implicit hc: HeaderCarrier, request: Request[_]): Future[Call] =
    http
      .post(url"${appConfig.addressLookupJourneyUrl}")
